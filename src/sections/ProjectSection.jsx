    ...
    return (
      ...
      >
        <div className="absolute inset-0">
          ...
          <div className="absolute top-6 right-6 w-2 h-2 bg-cyan-400 rounded-full animate-ping" style={{animationDelay: '0.5s'}}></div>
          <div className="absolute bottom-4 left-6 w-2 h-2 bg-cyan-400 rounded-full animate-ping" style={{animationDelay: '1s'}}></div>
        </div>
        <div className="text-center">
          <div className="w-12 h-12 mx-auto mb-3 border border-cyan-400/30 rounded flex items-center justify-center">
            <div className="text-cyan-400 font-mono text-xs font-medium">API</div>
          </div>
        </div>
      </div>
    );
...
const ProjectsSection = () => {
  ...
  const projects = [
    {
      title: "Dog API",
      description: "Interactive web application that fetches and displays random dog images using REST API integration",
      type: "dog-api",
    ...
    },
    {
      ...
      description: "Modern and fully responsive portfolio website showcasing projects with clean design and smooth animations",
      type: "portfolio",
      ...
      {fullscreenProject && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4">
          ...
          <div className="max-w-4xl w-full">
            ...
            />
            <h2 className="text-white text-2xl mt-6">{fullscreenProject.title}</h2>
            <p className="text-gray-300 mt-2">{fullscreenProject.description}</p>
          </div>
        </div>
      )}
    </section>
